...
function sna_show_notice_dashboard() {
    echo "<h2>" . __('Welcome to Show notice on Admin Dashboard plugin') . "</h2>";
    ...
    <div class="error_notice_area">
        <form name="error_notice" action="" method="post">
            <label><?php _e('Urgent Notice:'); ?></label> <textarea class="tex_ar_err" name="notice_urgt" rows="3" ><?php echo get_option('urgnt_notice'); ?></textarea><br>
            <input type="submit" name="urgnt_notice" value="<?php _e('Save'); ?>" class="notice_sub"> 
            <input type="submit" name="remove_ur_notice" value="<?php _e('Remove Notice'); ?>" class="notice_remv"> 
    ...
    <div class="nrml_notice_area">
        <form name="nrml_notice" action="" method="post">
            <label><?php _e('Normal Notice:'); ?> </label><textarea class="tex_ar_nrml" name="notice_nrml" rows="3"><?php echo get_option('nrml_notice'); ?></textarea><br>
            <input type="submit" name="nrml_notice" value="<?php _e('Save'); ?>" class="notice_sub">
            <input type="submit" name="remove_nr_notice" value="<?php _e('Remove Notice'); ?> " class="notice_remv"> 
        </form>
    </div>
...
function show_admin_notice() {
    ...
    if (!empty($urngent_notices)) {
        echo '<div class="error notice is-dismissible" id="message"><p>' . __($urngent_notices) . '</p></div>';
    }

    if (!empty($norml_notices)) {
        echo '<div class="updated notice is-dismissible" id="message"><p>' . __($norml_notices) . '</p></div>';
    }
}
